...
FROM
    users up
LEFT JOIN
    dependents d ON up.id = d.principal_id
LEFT JOIN
    users ud ON d.dependent_id = ud.id
WHERE
    up.role = 'UP';
...
FROM
    financial_goals fg
INNER JOIN
    users u ON fg.user_id = u.id
WHERE
    fg.dependent_id IS NULL;
...
FROM
    transactions
WHERE
    date BETWEEN '2025-02-01' AND '2025-02-07';
...
FROM
    transactions
WHERE
    category IN ('Alimentação', 'Transporte');
...
SELECT
    description,
    value,
    'Transação Avulsa' AS tipo_origem
FROM transactions
WHERE value > 500
  AND type = 'saida'
